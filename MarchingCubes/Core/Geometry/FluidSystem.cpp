...
//void FluidSystem::ApplyBoundaryConditions()
//{
//	const float e = m_params.restitution;   // 복원계수
//	const float muT = m_params.friction;      // 접선 감쇠
//	const float eps = m_params.penEps;        // 위치 보정
//	const float vE = m_params.velEps;        // 정지 클램프
//
//	for (auto& p : m_particles) {
//		XMVECTOR pos = XMLoadFloat3(&p.pos);
...
//			XMVECTOR vt = XMVectorSubtract(vel, XMVectorScale(n, vn));
//			vel = XMVectorAdd(XMVectorScale(n, -e * vn), XMVectorScale(vt, muT));
//			pos = XMVectorSet(m_domainMin.x + eps, XMVectorGetY(pos), XMVectorGetZ(pos), 0);
//		}
//		else if (px > m_domainMax.x) {
//			XMVECTOR n = XMVectorSet(-1, 0, 0, 0);
//			float vn = XMVectorGetX(XMVector3Dot(vel, n));
//			XMVECTOR vt = XMVectorSubtract(vel, XMVectorScale(n, vn));
//			vel = XMVectorAdd(XMVectorScale(n, -e * vn), XMVectorScale(vt, muT));
//			pos = XMVectorSet(m_domainMax.x - eps, XMVectorGetY(pos), XMVectorGetZ(pos), 0);
//		}
//
//		// Y 면
//		float py = XMVectorGetY(pos);
//		if (py < m_domainMin.y) {
//			XMVECTOR n = XMVectorSet(0, 1, 0, 0);
//			float vn = XMVectorGetX(XMVector3Dot(vel, n));
//			XMVECTOR vt = XMVectorSubtract(vel, XMVectorScale(n, vn));
//			vel = XMVectorAdd(XMVectorScale(n, -e * vn), XMVectorScale(vt, muT));
//			pos = XMVectorSet(XMVectorGetX(pos), m_domainMin.y + eps, XMVectorGetZ(pos), 0);
//		}
//		else if (py > m_domainMax.y) {
//			XMVECTOR n = XMVectorSet(0, -1, 0, 0);
//			float vn = XMVectorGetX(XMVector3Dot(vel, n));
//			XMVECTOR vt = XMVectorSubtract(vel, XMVectorScale(n, vn));
//			vel = XMVectorAdd(XMVectorScale(n, -e * vn), XMVectorScale(vt, muT));
//			pos = XMVectorSet(XMVectorGetX(pos), m_domainMax.y - eps, XMVectorGetZ(pos), 0);
//		}
//
//		// Z 면
...
//			XMVECTOR vt = XMVectorSubtract(vel, XMVectorScale(n, vn));
//			vel = XMVectorAdd(XMVectorScale(n, -e * vn), XMVectorScale(vt, muT));
//			pos = XMVectorSet(XMVectorGetX(pos), XMVectorGetY(pos), m_domainMin.z + eps, 0);
//		}
//		else if (pz > m_domainMax.z) {
//			XMVECTOR n = XMVectorSet(0, 0, -1, 0);
//			float vn = XMVectorGetX(XMVector3Dot(vel, n));
//			XMVECTOR vt = XMVectorSubtract(vel, XMVectorScale(n, vn));
//			vel = XMVectorAdd(XMVectorScale(n, -e * vn), XMVectorScale(vt, muT));
//			pos = XMVectorSet(XMVectorGetX(pos), XMVectorGetY(pos), m_domainMax.z - eps, 0);
//		}
//
//		// 너무 작은 떨림 제거
//		if (XMVectorGetX(XMVector3LengthSq(vel)) < vE * vE)
//			vel = XMVectorZero();
//
//		XMStoreFloat3(&p.pos, pos);
